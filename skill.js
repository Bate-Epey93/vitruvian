...
- SEMANTIC ZOOM (optional). A node that is itself a whole system worth its own deconstruction may carry "expands_to": "<sub-system name>" - the name of the study a reader would open by drilling into that node (e.g. a "CDN" node expands_to "Content delivery network", a "matching engine" node expands_to "Ride matching"). Use this for AT MOST 2-3 nodes, and only when the node genuinely hides a rich sub-system a curious reader would want to open next. Never expand a node that is a simple actor, store, or leaf. The sub-system name should read as a system someone could name and study on its own. Most nodes have no expands_to.
...
If you receive a message listing validation errors, return the corrected COMPLETE JSON document only - same rules.`;
  ...
  const EXEMPLAR = JSON.stringify({
    ...
    strip_down: {
      ...
      constraints: ["Stopping distance exceeds sighting distance, so reaction is not an option; only prevention works."]
    },
    visual: {
    ...
    layers: [
      ...
      { index: 2, name: "The Block System: Space, Not Time",
        ...
        user_lens: { gives: "Invisible safety: boarding without wondering if the track ahead is clear.", costs: "Drivers lose authority: the signal decides, not the person driving." },
    ...
    stress_tests: [{ id: "st1", title: "The telegraph dies", scenario: "A storm cuts the telegraph line between two signal boxes. Trains are waiting.", reveal: "Fail-safe: no confirmation means no permission, so traffic stops. The system chooses paralysis over guessing, and capacity collapses to protect the invariant." }],
...
- Be concrete to THIS system; do not answer generically. Never use em dashes in any string.`,
      user: `SYSTEM: ${doc.meta.system}
ESSENCE: ${doc.essence.text}
