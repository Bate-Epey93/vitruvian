...
/* ---------- FIELD_STUDIES · mechanisms that also exist in three dimensions ----------
   A Deconstruct whose subject is one of these gets a way across to the
   drawing. Only built studies belong here: an offer to open something that
   does not exist is worse than no offer.

   Terms are matched whole-word against the normalised system name, and every
   one of them is an unambiguous phrase on purpose — bare "transmission" is
   also what TCP does, and "gear" alone catches half the industrial world. */
var FIELD_STUDIES = [
  {
    id: "transmission",
    name: "The Gear Change",
    href: "lab/transmission.html",
    sub: "A manual gearbox, and the six failures that built it",
    blurb: "This one is also a machine. The same failure ladder, drawn in three dimensions: turn it over, cut it open, and shift it yourself.",
    terms: ["manual transmission", "manual gearbox", "car transmission",
            "gearbox", "gear box", "gear change", "synchromesh", "syncromesh",
            "stick shift", "manual shift", "dog clutch", "shift linkage"]
  }
];

/* → the study for a system name, or null. Pure; safe to call while rendering. */
function fieldStudyFor(system) {
  if (!system) return null;
  var s = " " + String(system).toLowerCase().replace(/[^a-z0-9]+/g, " ").trim() + " ";
  for (var i = 0; i < FIELD_STUDIES.length; i++) {
    for (var j = 0; j < FIELD_STUDIES[i].terms.length; j++) {
      if (s.indexOf(" " + FIELD_STUDIES[i].terms[j] + " ") >= 0) return FIELD_STUDIES[i];
    }
  }
  return null;
}
...
var COPY = {
  ...
  labChip: "3D study",
  labKicker: "Also a machine",
  labOpen: "Open the field study →",
